...
void dfs(int node, int from_node, int master_node, Info& info, int& total, set<int>& nodes, const int signalSpeed) {
    if (from_node != -1) {
    ...
    for (const auto& kv : adj_list[node]) {
        ...
        dfs(neigh, node, master_node, info, total, nodes, signalSpeed);
    }
...
public:
    vector<int> countPairsOfConnectableServers(vector<vector<int>>& edges, int signalSpeed) {
        ...
        for (int c=0; c<N; c++) {
            ...
            set<int> nodes;
            int total=0;
            dfs(c, -1, c, info, total, nodes, signalSpeed);
