class Solution {
    ...
    int signature(int x_left, int distance) {
        return ((x_left << 16) | distance);
    }
...
public:
    // To get it accepted, simply changed the signature from "string" type to "int" type.
    // That little additional efficiency was enough to push it over the edge. 
    // \_ o _/
    int minAreaRect(vector<vector<int>>& points) {
        unordered_map<int, vector<pair<Point, Point>>> check_map;
        ...
        for (int i=0; i<points.size(); i++) {
            ...
            for (int j=i+1; j<points.size(); j++) {
                ...
                int sig = signature(left_x, distance);
        ...
        for (const auto& kv : check_map) {
            ...
            for (int i=0; i<vec.size(); i++) {
                for (int j=i+1; j<vec.size(); j++) {
                    const Point& a_1 = vec[i].first; Point b_1 = vec[i].second;  // 1st line segment
                    const Point& a_2 = vec[j].first; Point b_2 = vec[j].second;  // 2nd line segment
