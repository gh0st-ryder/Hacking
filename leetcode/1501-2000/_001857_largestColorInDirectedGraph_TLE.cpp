class Solution {
    // The current solution gets a TLE in spite of memoization
    // 40/84 test cases pass.
    // 
    ...
    // 
    // NOTE: It is extremely important that this is a multimap
    // you can have multiple different terminal chains from "from" to "to"
    // where each path has different Result outcomes
    // We need to keep and consider them all.
    using TermResult = std::multimap<int, Result>;
        ...
        if (!adj_list[node].empty()) {
            for (int to : adj_list[node]) {
                ...
                for (auto& [key, value] : rec) {
                    ...
                    // and store this (possibly duplicate) key in my term results.
                    my_term.insert({key, value});
...
public:
    int largestPathValue(string colors_in, vector<vector<int>>& edges) {
        ...
        for (const auto& edge : edges) {
            ...
            max_node = std::max(max_node, edge[0]);
            max_node = std::max(max_node, edge[1]);
        }
